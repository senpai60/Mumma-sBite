...
export const createOrderHandler = async (req, res) => {
  const { currency = "INR", receipt, cartId, deliveryDetails } = req.body;
  ...
  try {
    ...
    // An order is useless to fulfil without somewhere to send it.
    const missingField = ["fullName", "mobile", "address"].find(
      (field) => !String(deliveryDetails?.[field] || "").trim()
    );
    if (missingField) {
      return res.status(400).json({
        success: false,
        message: `Delivery details are incomplete (${missingField} is required)`,
      });
    }

    // Load the cart and confirm it belongs to the caller — a cartId from the
    // client is just a string, it proves nothing about ownership.
    ...
    // Saving re-runs the cart's pre-save hook, which recomputes subtotal/gst/
    // delivery/grandTotal from current product prices. This is the only source
    // of the payable amount — the client never sends a price.
    await cart.save();

    const orderAmountInPaise = Math.round((cart.grandTotal || 0) * 100);
    ...
    if (orderProducts.length === 0) {
      return res.status(400).json({
        success: false,
        message: "Cart has no orderable products",
      });
    }
    ...
    // Persist before responding. If this throws, the client never receives the
    // Razorpay order id, so nothing payable escapes without a matching record.
...
orderRouter.post("/create-order", protect, createOrderHandler);
orderRouter.post("/verify", protect, verifyPaymentHandler);
orderRouter.post("/verify-payment", protect, verifyPaymentHandler);
